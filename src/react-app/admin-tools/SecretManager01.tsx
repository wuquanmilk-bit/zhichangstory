...
import { 
  ...
  Video, Play, ChevronLeft, ExternalLink
} from 'lucide-react';
...
const SecretManager = () => {
  ...
  // --- 内容监控（含视频） ---
  const [contentTab, setContentTab] = useState<'questions' | 'answers' | 'novels' | 'videos'>('questions'); // 增加videos
    ...
    try {
      ...
      const targetTable = isUserContent 
        ? (activeTab === 'questions' ? TABLE.QUESTIONS : 
           activeTab === 'answers' ? TABLE.ANSWERS : 
           TABLE.NOVELS)
        : contentTab === 'videos' ? TABLE.VIDEOS : contentTab;
  ...
  const handleBatchAudit = async (status: 'approved' | 'rejected') => {
    if (selectedContentIds.length === 0) return alert('请选择要审核的视频');
    if (!confirm(`确定要${status === 'approved' ? '通过' : '拒绝'}选中的 ${selectedContentIds.length} 条视频吗？`)) return;
    ...
    try {
      ...
      // 刷新数据
      loadAllContent();
      alert(`成功${status === 'approved' ? '通过' : '拒绝'}${selectedContentIds.length}条视频`);
    } catch (e: any) {
  ...
  const handleMouseDown = (index: number, itemId: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      const targetTable = contentTab === 'videos' ? TABLE.VIDEOS : contentTab;
  ...
  const handlePublishQuestion = async () => {
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const content = customContent || `这是自动生成的问题内容`;
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-200">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <Shield className="text-blue-600" /> 超级管理后台 <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">DB适配版（含视频管理）</span>
          </h1>
        </div>
                  ...
                  <div className="space-y-4 pt-4">
                     ...
                     <div className="flex gap-2">
                       <input 
                         ...
                         onChange={e => setCustomCoinAmount(Number(e.target.value))} 
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className={`px-3 py-1 text-sm rounded ${contentTab === 'videos' ? 'bg-purple-100 text-purple-600' : 'bg-gray-100'}`}
              >
                <Video size={14} className="inline mr-1" /> 视频
              </button>
            </div>
          </div>
          ...
          {contentTab === 'videos' && (
            <div className="mb-4 flex gap-2">
              <button 
                ...
                disabled={selectedContentIds.length === 0 || loading}
                className={`px-3 py-1 bg-green-100 text-green-600 border border-green-200 rounded text-sm ${selectedContentIds.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                批量通过
              </button>
              <button 
                ...
                disabled={selectedContentIds.length === 0 || loading}
                className={`px-3 py-1 bg-red-100 text-red-600 border border-red-200 rounded text-sm ${selectedContentIds.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                批量拒绝
              </button>
            </div>
                  ...
                  {allContentData.map((item, index) => (
                    <tr 
                      ...
                      className={`border-b hover:bg-gray-50 ${
                        dragSelecting && dragStartIndex !== null && dragEndIndex !== null && 
                        index >= Math.min(dragStartIndex, dragEndIndex) && 
                        index <= Math.max(dragStartIndex, dragEndIndex) 
                          ? 'bg-blue-50' 
                          : ''
                      }`}
                      onMouseDown={(e) => handleMouseDown(index, item.id, e)}
                    ...
                    >
                      ...
                      {contentTab === 'videos' && (
                        <>
                          <td className="p-2 truncate max-w-md">{item.title || '无标题'}</td>
                          <td className="p-2">
                            <img 
                          ...
                          <td className="p-2 text-gray-500">{item.views_count || 0}</td>
                          <td className="p-2 text-xs text-gray-400">{new Date(item.created_at).toLocaleString()}</td>
